...
class AccountingControlPolicyTest extends TestCase
{
    ...
    {
        $warehouse = new User(['type' => User::ROLE_PURCHASING]);
    ...
    public function test_every_authenticated_user_can_view_material_but_only_accounting_sees_price(): void
    {
        ...
        $warehouse = new User(['type' => User::ROLE_PURCHASING]);
        $accounting = new User(['type' => User::ROLE_ACCOUNTING]);
        $other = new User(['type' => User::ROLE_FINANCE]);
    ...
    {
        $policy = new FakturPembelianPolicy();
        $paymentUser = new User(['type' => User::ROLE_FINANCE]);
        $invoice = new FakturPembelian(['status' => FakturPembelian::UNPAID, 'sisa_tagihan' => 100000]);
