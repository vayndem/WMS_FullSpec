...
@section('content')
    <div class="content-page">
        <div class="mb-4 flex flex-wrap items-center justify-between gap-2">
            <div>
                <h3 class="text-2xl font-bold">Asset Tetap</h3>
                <p class="text-base-content/60">Register fisik, nilai buku, dan penyusutan asset perusahaan.</p>
            </div>
            @can('create', App\Models\Asset::class)
                <div class="flex gap-2">
                    <a class="btn btn-outline btn-primary" href="{{ route('asset-categories.index') }}">Kategori & COA</a>
                    <a class="btn btn-primary" href="{{ route('assets.create') }}"><i class="fa-solid fa-plus"></i> Tambah Asset</a>
                </div>
            @endcan
        </div>
        <div class="card border border-base-300 bg-base-100 shadow-sm">
            <div class="p-4">
                <form class="grid grid-cols-1 gap-2 md:grid-cols-6">
                    ...
                    <input name="q" value="{{ request('q') }}" class="input input-bordered md:col-span-2" placeholder="Cari asset..." onchange="this.form.submit()">
                    ...
                    <a class="btn btn-error md:col-span-2" href="{{ route('assets.report.pdf', request()->query()) }}"><i class="fa-solid fa-file-pdf"></i> PDF</a>
                </form>
            </div>
                ...
                <table class="table" data-row-start="{{ $assets->firstItem() ? $assets->firstItem() - 1 : 0 }}">
                    ...
                    <tbody>
                        @forelse($assets as $asset)
                            <tr>
                                <td class="font-semibold text-primary">{{ $asset->asset_number }}</td>
                                <td>{{ $asset->name }}</td>
                                ...
                                <td class="text-end"><a href="{{ route('assets.show', $asset) }}" class="btn btn-outline btn-primary btn-sm">Detail</a></td>
                            </tr>
                        @empty
                            <tr><td colspan="10" class="py-4 text-center text-base-content/50">Belum ada asset.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
