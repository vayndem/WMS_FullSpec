...
The next concrete, unresolved things this file points at are the **"Known gaps" list** above (analyzed 2026-09-01, updated for the new names in the docs-pass commit) — still-open items include: no multi-currency/kurs handling, no AR/piutang or outbound-to-customer side at all, no real barcode/RF scanning, no wave/batch picking or slotting, no BOM/manufacturing/WIP model, no external API/EDI integration, no kitting/bundling, and the residual retur-after-invoice-posted case. None of these are part of the rename work — they're the pre-existing product backlog, unaffected by this sweep. Ask the user which (if any) they want tackled next rather than assuming.
