...
### Cluster 10 — WMS control framework tables: not yet started, largest remaining cluster

Not yet scoped in detail. Known table/model set from `feed.MD`'s WMS control framework section: `PickingOrder`, `QualityInspection` (+ a details/lines table), `InventoryLayer`, `InventoryLot`, `InventorySerial`, `InventoryReservation`, `ReplenishmentSuggestion`, `WarehouseLocation`, `AuditLog`, `DocumentReversal`, plus `LandedCost`/`LandedCostAllocation` (seen referenced from the ChartOfAccount cluster's FK list, `credit_coa_id`/`journal_id` on `landed_costs`) — confirm the exact set and whether any belong together as one migration/feature (like Lpb+Bap or ServicePurchase+ServiceCategory did) before assuming they're 10 independent sub-clusters. Given the size, spawn an Explore agent first the same way clusters 3–9 did, and consider whether this genuinely needs one combined pass or should be split into 2–3 sequential sub-clusters (e.g. traceability tables together, execution/reservation tables together, financial-control tables together) to keep each `migrate:fresh`+test+commit cycle reviewable. Watch for the same landmines hit in prior clusters: implicit `hasMany()`/`hasOne()` FK guessing on any renamed class, raw `DB::table()` calls with aliases, MySQL's 64-char identifier limit on longer `wms_`-prefixed names, and plain-UI-text collisions if any of these entities' English names double as ordinary prose elsewhere.

### Cluster 11 — Docs pass: not started

Once cluster 10 is done, sweep `README.md`, `feed.MD`, and this file's own non-checkpoint sections (the "Domain concepts" and "Architecture rules" content above the Phase 2/3 checkpoint, which still describe pre-rename table/class names) for old terminology so documentation matches the renamed code.

To resume: re-read this checkpoint, `git log --oneline` on `refactor/rename-baku-indonesia` to see exactly what's committed, `git status` to see any uncommitted WIP, then continue with cluster 10. Before starting a cluster, it's worth spawning a read-only Explore agent to map the cluster's full footprint first (migrations, model, every consumer file, raw table-name strings, routes, views, seeders, tests) the way clusters 3 through 9 did — it catches things a quick manual grep misses. No need to ask the user before continuing — full autonomy for this rename work was already granted for as long as this checkpoint stands; re-confirm with the user only if resuming in a context where that grant is unclear (e.g. a very different session much later). The user has also said explicitly not to push this branch anywhere — everything stays local so they can review progress themselves.
