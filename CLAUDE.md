...
- No statutory financial statement reports — no Neraca Saldo (trial balance), Buku Besar per account, Laporan Laba Rugi, or Neraca (balance sheet). Only a raw journal list (`JurnalController::reportPdf`) and the reconciliation dashboard exist. The underlying `wms_jurnal_detail` data supports building these; the report layer just doesn't exist yet. **Still open** — biggest remaining accounting-completeness gap.
