...
### Cluster 2 — LPB/BAP: 🚧 IN PROGRESS, currently BROKEN on the branch (migrations + models done, ~65 consumer files not yet updated)

This is the highest-fan-in table in the schema (procurement + warehouse + invoicing + three-way match + reversal + every seeder touches it), and `ServiceBap extends Lpb` (a real PHP subclass, sharing the same table via a `document_type` discriminator column: `'GOODS'` vs `'SERVICE_BAP'`) — so LPB and BAP had to be done as one combined cluster.

**Already done and committed** (commit "WIP: LPB/BAP cluster rename..."):
- Tables: `lpbs` → `wms_penerimaan_barang`, `lpb_details` → `wms_penerimaan_barang_detail`, `service_bap_details` → `wms_penerimaan_jasa_detail`, `service_bap_allocations` → `wms_penerimaan_jasa_alokasi`. All 9 migration files that reference these tables (create + every later `Schema::table(...)`/`->on(...)`/`->constrained(...)`/raw `DB::table(...)` join/select) have been updated — verified with a full grep sweep, zero old table-name references remain in `database/migrations/`. Columns (`id_lpb`, `lpb_id`, `lpb_detail_id`, `no_po`, etc.) were deliberately left unchanged.
- Models renamed and rewritten: `app/Models/Lpb.php` → `PenerimaanBarang.php` (class `PenerimaanBarang`, `$table = 'wms_penerimaan_barang'`, constants `DRAFT`/`POSTED`/`REVERSED`/`CANCELLED` kept as-is), `LpbDetail.php` → `PenerimaanBarangDetail.php` (`$table = 'wms_penerimaan_barang_detail'`), `ServiceBap.php` → `PenerimaanJasa.php` (now `class PenerimaanJasa extends PenerimaanBarang`, same global scope/creating-hook logic), `ServiceBapDetail.php` → `PenerimaanJasaDetail.php` (`$table = 'wms_penerimaan_jasa_detail'`), `ServiceBapAllocation.php` → `PenerimaanJasaAlokasi.php` (`$table = 'wms_penerimaan_jasa_alokasi'`). All internal cross-references between these 5 files already point to the new class names.

**NOT done yet — the repo will not pass `php artisan test` until these are updated.** Every file below still references the old class names (`Lpb`, `LpbDetail`, `ServiceBap`, `ServiceBapDetail`, `ServiceBapAllocation`) via `use` statements, type hints, `::class`, or `new`/static calls, and needs the mechanical rename applied (rename the class reference only — do NOT rename local variable names like `$lpb`, `$detail`, per the route-binding rule above):

- **Controllers**: `app/Http/Controllers/LpbController.php` → rename file to `PenerimaanBarangController.php`, class `PenerimaanBarangController`; `app/Http/Controllers/ServiceBapController.php` → `PenerimaanJasaController.php`, class `PenerimaanJasaController`. Also check `WarehouseExecutionController.php`, `InventoryFinancialControlController.php`, `ReturPembelianController.php`, `RequestDetailController.php`, `InvoiceLpbController.php`, `WmsControlController.php`, `AccountingReconciliationController.php`, `AuthController.php` for `Lpb`/`ServiceBap` references (type hints, `Lpb::class`, `Lpb::POSTED` etc.) — update type hints only, keep variable names.
- **Policies**: `app/Policies/LpbPolicy.php` → `PenerimaanBarangPolicy.php`; also update `app/Policies/ServiceBapPolicy.php` → `PenerimaanJasaPolicy.php`, `ServicePurchasePolicy.php` (references `ServiceBap`).
- **Requests**: `StoreLpbRequest.php`→`StorePenerimaanBarangRequest.php`, `UpdateLpbRequest.php`→`UpdatePenerimaanBarangRequest.php`, `StoreLpbDetailRequest.php`→`StorePenerimaanBarangDetailRequest.php`, `UpdateLpbDetailRequest.php`→`UpdatePenerimaanBarangDetailRequest.php`, `StoreServiceBapRequest.php`→`StorePenerimaanJasaRequest.php`, `CancelServiceBapRequest.php`→`CancelPenerimaanJasaRequest.php`. Also check `UpdateInvoiceLpbRequest.php`, `StoreReturPembelianRequest.php`, `StoreInvoiceLpbRequest.php`, `UpdateGudangRequest.php` for `Lpb`/`id_lpb` type references.
- **Services**: `WmsAccountingService.php`, `WarehouseExecutionService.php`, `ThreeWayMatchService.php`, `StockOpnameService.php`, `InventoryReversalService.php` all reference `Lpb`/`LpbDetail`/`ServiceBap` — update `use` statements and type hints only.
- **Models still referencing old names**: `ReturPembelian.php`, `QualityInspection.php`, `Pembelian.php`, `InvoiceLpb.php`, `InvoiceLpbReceipt.php`, `Gudang.php` — update relation type hints (e.g. `belongsTo(Lpb::class)` → `belongsTo(PenerimaanBarang::class)`).
- **Routes**: `routes/warehouse.php` — resource `lpb` → `penerimaan-barang` with `->parameters(['penerimaan-barang' => 'lpb'])` to keep `{lpb}` bindings and route names like `lpb.get-po-detail`→`penerimaan-barang.get-po-detail`, `lpb.report.pdf`→`penerimaan-barang-report/pdf` + name `penerimaan-barang.report.pdf`, `lpb.details.store/update/destroy`, `lpb.inspect`, `lpb.putaway`, `lpb.reverse` all get the `penerimaan-barang.*` name prefix (URL segments can keep `lpb/...` as a sub-path or also become `penerimaan-barang/...` — pick one and update the matching route()/action() calls in views/JS consistently). `routes/procurement.php`/`routes/finance.php` — check `retur-pembelian.get-lpb-detail`, `invoice-lpb.get-lpb-detail` (these reference `{id_lpb}` param, a column name — leave the param name, just verify controller still resolves). `routes/assets-and-services.php` — resource `service-baps` → `penerimaan-jasa` with `->parameters(['penerimaan-jasa' => 'service_bap'])`, route `service-baps.cancel`→`penerimaan-jasa.cancel`, `service-baps.report.pdf`→`penerimaan-jasa.report.pdf`.
- **Views**: rename directory `resources/views/lpb/` → `resources/views/penerimaan_barang/` (index, create, show-page, and check for others), `resources/views/service_baps/` → `resources/views/penerimaan_jasa/` (index, create, show). Update every `route('lpb.*')`/`route('service-baps.*')` call and `view('lpb.*')`/`view('service_baps.*')` reference across ALL views, not just these two directories — also check `invoice_lpb/create.blade.php`, `invoice_lpb/edit.blade.php`, `retur_pembelian/create.blade.php`, `retur_pembelian/index.blade.php`, `wms_control/index.blade.php`, `purchasing/dashboard.blade.php`, `gudang/dashboard.blade.php`, `accounting_reconciliation/show.blade.php`, `service_purchases/show.blade.php`, `layouts/template/sidebar.blade.php` (has an LPB nav entry with `routeIs('lpb.*')`/`routeIs('service-baps.*')` — same pattern as the Aset sidebar fix already done).
- **Seeders**: `database/seeders/WmsTransactionScenarioSeeder.php`, `AssetAndServiceDemoSeeder.php`, `FinancialStatementDemoSeeder.php` — all create `Lpb`/`ServiceBap` records, update `use` + `Lpb::class`/`new Lpb(...)` references. (`WmsDemoSeeder` didn't show up in the grep — double check it too, it may reference LPB indirectly.)
- **Tests**: `tests/Feature/WmsControlFrameworkTest.php`, `tests/Unit/WarehouseRolePolicyTest.php`, `tests/Unit/AssetAndServicePolicyTest.php` (already partly touched in cluster 1 — it has `use App\Models\Lpb;`/`Lpb::POSTED` left over) all reference `Lpb`/`ServiceBap` — update class references, keep test logic identical.
- After all of the above: `php artisan migrate:fresh --seed --force` → `php artisan test` → `composer test:case-sensitive` → fix whatever the test failures point at (there will likely be a few missed spots; the test suite has strong coverage of exactly this reversal/three-way-match/reconciliation logic, so trust its failures) → commit as a clean (non-WIP) commit → move to the next cluster.

### Remaining clusters (not started, roughly in suggested order)

3. **ServicePurchase/ServiceCategory** (the PO-side of services, coupled to PenerimaanJasa via `ServicePoDetail`) — e.g. `ServicePurchase`→`PesananJasa`, `ServiceCategory`→`KategoriJasa`, tables `service_purchases`→`wms_pesanan_jasa`, `service_categories`→`wms_kategori_jasa`, `service_po_details`→`wms_pesanan_jasa_detail`.
4. **Pembelian** (Purchase Order) → consider `PesananPembelian`, table `pembelians`→`wms_pesanan_pembelian` (currently already Indonesian-ish; lower priority/less jargon-y than LPB/BAP/NPK, but user said all DB names should become Indonesian).
5. **Npk** (material usage note) → `PemakaianBarang`, table `npks`→`wms_pemakaian_barang`, `npk_stock_allocations`→`wms_pemakaian_barang_alokasi_stok`.
6. **InvoiceLpb/InvoicePayment** (supplier invoice/payment) → e.g. `InvoiceLpb`→`FakturPembelian`, `InvoicePayment`→`PembayaranFaktur`.
...
To resume: re-read this checkpoint, `git log --oneline` on `refactor/rename-baku-indonesia` to see exactly what's committed, `git status` to see any uncommitted WIP, then continue with cluster 2's remaining file list above. No need to ask the user before continuing — full autonomy for this rename work was already granted for as long as this checkpoint stands; re-confirm with the user only if resuming in a context where that grant is unclear (e.g. a very different session much later).
