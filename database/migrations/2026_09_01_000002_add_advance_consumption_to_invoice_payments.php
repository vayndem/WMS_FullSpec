    ...
    public function up(): void
    {
        Schema::table('wms_pembayaran_faktur', function (Blueprint $table) {
            $table->unsignedBigInteger('uang_muka_sumber_payment_id')->nullable()->after('kelebihan_pembayaran')
                ->comment('Referensi ke wms_pembayaran_faktur.id yang menjadi sumber uang muka supplier, bila pembayaran ini memakai uang muka');
            ...
            $table->foreign('uang_muka_sumber_payment_id')->references('id')->on('wms_pembayaran_faktur')->onDelete('restrict');
        });
    }
